...
if __name__ == "__main__":
    ...
    #train_data = DataProcessorLogistic.CustomCIFAR10(train_dataset)
    ...
    #test_dataset_DRO = PGD_attack.generate_fgsm_attacked_dataset(model_DRO, sub_sampled_test_data, epsilon)
    #test_dataset_ERM = PGD_attack.generate_fgsm_attacked_dataset(model_ERM, sub_sampled_test_data, epsilon)
    #test_dataset_fDRO = PGD_attack.generate_fgsm_attacked_dataset(model_fDRO, sub_sampled_test_data, epsilon)
    
    #outer_loss_list_base_DRO = train.sinkhorn_wang_train(model_DRO_base, model_DRO_base.lr1, train_dataloader, num_epochs, checkpoint_dir, interval)
    
    outer_loss_list = train.sinkhorn_train(model_DRO,model_DRO.lr1,eta0_list.clone(),train_dataloader, num_epochs, checkpoint_dir, interval)
    #outer_loss_list_ERM = train.ERM_train(model_ERM,ERM_lr, train_dataloader, num_epochs, checkpoint_dir, interval)
    #outer_loss_list_fDRO = train.fDRO_train(model_fDRO, model_fDRO.lr1, train_dataloader, num_epochs, checkpoint_dir, eta_fDRO, interval)
    ...
    #outer_loss_linear = [ loss.cpu().item() for loss in outer_loss_list_ERM]
    ...
    #fig=plt.figure()
    #plt.loglog(epochs_list, test_loss_DRO,'-o',color='red',label="SDRO "+f" fgsm {epsilon}")
    ...
    #plt.loglog(epochs_list, test_loss_fDRO,'-*',color = 'green', label = "fDRO "+ f" fgsm {epsilon}")
    ...
    #plt.loglog(epochs_list, test_loss_linear,'-+', color = 'blue',label = "ERM "+ f" fgsm {epsilon}")
    ...
    #plt.legend()
    #plt.show()
    #fig.savefig(f'test_LeNet_{epsilon}.png')
